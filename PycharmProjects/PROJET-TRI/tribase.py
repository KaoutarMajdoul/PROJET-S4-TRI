...
def radixSort(nbElem):
    ...
    list_vals = random.sample(range(100,100000), nbElem)  # On créé une liste de nbElem éléments de valeurs aléatoires inférieures ou égale à 1000
    ...
    nbOccurence = [0 for i in range(0, max(
        list_vals) + 1)];  # On créé un tableau de taille max(list_vals)+1  dans lequel on placera le nombre d'occurence à un indice donnée
    buckets2 = [[] for y in range(0, max(
        list_vals) + 1)]  # On creéé un tableau ayant pour indice les valeurs du tableau nbOccurence
    listTmp = [0 for i in range(0, len(list_vals))]
    while not maxLength:
        list_counts = [int(((list_vals[i] % (placement * 10)) / placement)) for i in range(0,
                                                                                           nbElem)]  # On complète le tableau list_count en commençant par le chiffre des unités des valeurs de list_vals
        ...
        completePathTemps = os.path.join(savepathTemps, '%dtribase.csv ' % nbElem)
